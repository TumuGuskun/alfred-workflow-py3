...
def fold_to_ascii(text: str) -> str:
    ...
    Returns:
        str: text containing only ASCII characters
    """
    if isascii(text):
        return text
    text = ''.join([ASCII_REPLACEMENTS.get(c, c) for c in text])
    return strdata.normalize('NFKD', text).encode('ascii', 'ignore').decode()
...
def isascii(text):
    ...
    try:
        text.encode('ascii')
    except strEncodeError:
        return False
    return True


def dumbify_punctuation(text: str) -> str:
